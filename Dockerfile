...
RUN apt-get update --fix-missing && \
    apt-get install -y sudo wget bzip2 ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 git mercurial subversion nodejs npm && \
    apt-get clean && \
...
RUN wget --quiet https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O ~/miniconda.sh && \
    ...
    . ~/.bashrc && \
    /bin/bash -c "conda install jupyter jupyterlab conda-build -y" --quiet && \
